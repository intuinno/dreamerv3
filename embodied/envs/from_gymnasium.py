...
class FromGymnasium(embodied.Env, Generic[U, V]):
        ...
        **kwargs,
        ...
        if isinstance(env, str):
            self._env: gymnasium.Env[U, V] = gymnasium.make(
                env, render_mode=render_mode, **kwargs
            )
        else:
            assert not kwargs, kwargs
            # For existing env instances, we can't change render_mode
            # but we store the desired mode for render() calls
            if hasattr(env, 'render_mode') and env.render_mode != render_mode:
                print(f"Warning: Environment has render_mode={env.render_mode}, but {render_mode} was requested")
            self._env = env
    ...
    def render(self):
        if self._render_mode == "human":
            # For human mode, just call render and return None
            self._env.render()
            return None
        else:
            # For rgb_array mode, return the image array
